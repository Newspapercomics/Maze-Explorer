  ...
  def explore(self):
    ...
    while current_room != "treasure room":
      ...
      for connected_room, weight in node.edges.items():
        ...
        print("enter {0} for {1}: {2} cost"
.fromat(key, connected_room, weight))
      valid_choices = [room[0] for room in node.edges.keys()]
      print("\nYou have accumulated: {0} cost".format(path_toal))
...
def build_graph():
  ...
  ante_chamber = Vertex("ante-chamger")
  ...
  graph.add_ege(entrance, ante_chamber, 7)
